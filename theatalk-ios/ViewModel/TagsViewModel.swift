...
class TagsViewBaseModel:ObservableObject,ViewModelErrorHandle{
    ...
    @Published var SearchText:String = ""{
        didSet{
            searchTags(tagName: SearchText)
        }
    }
    var tagFetcher = TagFetcher()
    init(Id:Int?){
        self.Id = Id
...
class TagsViewModel: TagsViewBaseModel{
    ...
    func getUserTags(){
        tagFetcher.GetUserTags(userId: self.Id!)
            .receive(on: DispatchQueue.main)
            .sink(
          receiveCompletion: { [weak self] value in
            guard let self = self else { return }
